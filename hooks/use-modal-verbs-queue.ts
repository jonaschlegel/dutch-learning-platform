import { type ModalVerb, modalVerbs } from '@/data/modal-verbs';
import { useCallback, useState } from 'react';

interface ModalVerbsQueueConfig {
  maxRecentItems?: number;
  prioritizeIncorrect?: boolean;
  focusLevel?: 'A1' | 'A2' | 'B1' | null;
  exerciseMode?:
    | 'conjugation'
    | 'usage'
    | 'translate'
...
export function useModalVerbsQueue(config: ModalVerbsQueueConfig = {}) {
  const {
    maxRecentItems = 5,
  ...
  const [currentQueue, setCurrentQueue] = useState<ModalVerb[]>([]);
  ...
  const initializeQueue = useCallback(
    (incorrectWords: Record<string, number> = {}, startIndex?: number) => {
      ...
      if (prioritizeIncorrect && Object.keys(incorrectWords).length > 0) {
        ...
        const incorrectModalVerbs = availableWords.filter((word) =>
          incorrectWordIds.includes(word.id),
        );

        if (incorrectModalVerbs.length > 0) {
          availableWords = incorrectModalVerbs;
        }
      }
    ...
    [focusLevel, prioritizeIncorrect],
  );
