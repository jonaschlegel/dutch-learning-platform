    ...
    <>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <Card className="bg-card shadow-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">
              {Object.keys(progress.incorrectWords).length}
            </div>
            <p className="text-xs text-muted-foreground">
              {Object.keys(progress.incorrectWords).length === 0
                ? 'No mistakes yet!'
                : 'Need more practice'}
            </p>
            {Object.keys(progress.incorrectWords).length > 0 && (
              <Button
                size="sm"
                variant={progress.mistakeMode ? 'default' : 'outline'}
                onClick={onToggleMistakeMode}
                className="mt-2 w-full text-xs"
              >
                {progress.mistakeMode ? 'Exit Review' : 'Review Mode'}
              </Button>
            )}
          </CardContent>
        </Card>
